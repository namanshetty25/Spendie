...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not is_upi_screenshot(extracted_text):
            await processing_msg.edit_text(
                "❌ *Not a UPI screenshot*\n"
        ...
        if not transaction_data:
            await processing_msg.edit_text(
                "⚠️ *Could not parse transaction*\n"
                "Unable to extract transaction details from the screenshot.",
                parse_mode="Markdown"
            )
            return
        ...
        success_message = f"{confidence_emoji} *UPI Transaction Added:*\n\n"
        ...
        await processing_msg.edit_text(
            ...
            "Something went wrong while processing your UPI screenshot.",
            parse_mode="Markdown"
        )
...
async def ocr_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if TESSERACT_AVAILABLE:
        ...
        status_message += "📱 UPI Screenshot support: Enabled\n"
    else:
        status_message += "❌ Tesseract OCR: Not installed\n"
        status_message += "📱 UPI Screenshot support: Limited\n"
...
def run_bot():
    ...
    app.add_handler(CommandHandler("ocr_status", ocr_status))
